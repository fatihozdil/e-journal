...
CKEDITOR.plugins.add("video", {
  ...
  onLoad: function () {
    ...
    CKEDITOR.addCss(
      "img.cke-video{background:#f8f8f8 url(" +
        url +
        ") center center no-repeat;outline:1px solid #ccc;outline-offset:-1px;min-width:192px;min-height:108px;max-width:100%;width:auto!important;height:auto!important;}"
    );
  },
  init: function (editor) {
    editor.addCommand(
      ...
      new CKEDITOR.dialogCommand("video", {
        allowedContent:
          "video[autoplay,controls,height,loop,muted,preload,!src,width]",
      })
    );
      ...
      dataFilter.addRules({
        elements: {
          "cke:video": function (element) {
            ...
            element = editor.createFakeParserElement(
              element,
              "cke-video",
              "video",
              false
